...
FP16 FP16::operator+(const FP16& other) const {
    ...
    //     return FP16(uint16_t(pzero));     
        ...
        //     if(is_large_a == 1) return FP16(value);
        //     else                return other;
        // }          

        
        // uint16_t post_e = (is_large_a == 1) ? other.get_expo() : get_expo();
        // // Normalize mantiassa 
        // uint32_t pre_m_a = (is_large_a == 1) ? (uint32_t)get_emani() << pre_diff :
        //                                        (uint32_t)get_emani();
        // uint32_t pre_m_b = (is_large_a == 1) ? (uint32_t)other.get_emani() :
        //                                        (uint32_t)other.get_emani() << pre_diff;    
        ...
        // uint16_t nor_s,nor_e,nor_m;
        // nor_s = post_s;
        // // Find position where the msb is 'one' ([10:0] << 31 => [42:0])
        // uint16_t msb_one_pos;
        // for(uint32_t pos=0;pos<63;pos++) {
        //     if((post_m >> pos & 0x1) == 1) msb_one_pos = pos;
        // }        

        // if(msb_one_pos >= 11) {
        //     uint16_t shift_pos = msb_one_pos - 11;
        //     nor_e = post_e + shift_pos;
        //     nor_m = (post_m >> shift_pos) & M_MASK;
        //     if(nor_e > E_MAX) {
        //         if(nor_s == 0) return FP16(uint16_t(pInf));
        //         else           return FP16(uint16_t(nInf));
        //     }
        //     // uint16_t nor_grs;
        //     // uint16_t nor_round_up_bits;
        //     // nor_grs = (post_m << (23-shift_pos)) & 0xFFFFFF;
        //     // nor_round_up_bits = get_round_bits(nor_grs);    
        //     // nor_m+=nor_round_up_bits;        
        // } else {
        //     uint16_t shift_pos = 11-msb_one_pos;
        //     nor_e = post_e - shift_pos;
        //     nor_m = (post_m << shift_pos) & M_MASK;     
        //     if(post_e <= shift_pos) {
        //         if(nor_s == 0) return FP16(uint16_t(pzero));
